...
const MapComponent: React.FC<MapComponentProps> = ({ layerTrigger }) => {
    ...
    useEffect(() => {
        ...
        if (mapDiv.current) {
            resizeObserver = new ResizeObserver(() => {
                ...
                // Esperamos estabilidad real del layout antes de inicializar
    ...
    return (
        // [SOLUCIÓN RESPONSIVE BASADA EN EL HALLAZGO]
        // Se extirpó el parche de `height: 500px` y `backgroundColor: red`.
        // En su lugar, usamos la cadena `flex: 1` combinada con `height: 100%` y `min-height: 0`.
        // Esto le da a Safari/WebKit la directiva explícita de calcular los píxeles reales 
        // y heredarlos dinámicamente, obteniendo el mismo efecto que el "500px" pero responsive.
        <div className="map-component-root" style={{ display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0, height: '100%', position: 'relative', width: '100%' }}>
            
            {/* El lienzo del mapa asume un 'flex: 1' y 'height: 100%' completamente limpio */}
            <div id="viewDiv" ref={mapDiv} style={{ flex: 1, minHeight: 0, height: '100%', position: 'relative', width: '100%' }}></div>
