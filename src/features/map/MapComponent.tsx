...
const MapComponent: React.FC<MapComponentProps> = ({ layerTrigger }) => {
  ...
  // Contenedores para montar widgets nativos del SDK dentro de la UI custom
  ...
  useEffect(() => {
    ...
    let delayedInitTimeout1: ReturnType<typeof setTimeout> | null = null;
    let delayedInitTimeout2: ReturnType<typeof setTimeout> | null = null;
    let rafId: number | null = null;
    let isInitialized = false;

    // Programa un intento de inicialización con debounce.
    // La idea es no crear el mapa mientras el layout móvil siga "respirando".
    ...
    // Intenta inicializar solo cuando el contenedor tenga un tamaño útil real.
    // No basta con > 0; exigimos un mínimo razonable para evitar arranques torcidos.
    const attemptInitialization = () => {
      if (!mapDiv.current || isInitialized) return;

      const width = mapDiv.current.offsetWidth;
      const height = mapDiv.current.offsetHeight;

      // Umbral mínimo práctico para considerar que el contenedor ya está listo.
      const hasUsableSize = width >= 200 && height >= 250;
      ...
      // Montaje de widgets en contenedores React
      ...
      // Una vez inicializado correctamente, dejamos de observar.
      if (resizeObserver) {
        resizeObserver.disconnect();
      }
    };

    // Observador principal del contenedor del mapa
    if (mapDiv.current) {
      ...
      // Primer disparo tras el primer paint
      rafId = window.requestAnimationFrame(() => {
        scheduleInitialization();
      });

      // Intentos de seguridad tras reflow/reacomodo móvil
      delayedInitTimeout1 = setTimeout(() => {
        scheduleInitialization();
      }, 300);

      delayedInitTimeout2 = setTimeout(() => {
        scheduleInitialization();
      }, 800);
    }

    // Reintentos por cambios reales del viewport móvil
    ...
    // Algunos móviles ajustan altura útil a través de visualViewport
    ...
    return () => {
      ...
      if (delayedInitTimeout1) clearTimeout(delayedInitTimeout1);
      if (delayedInitTimeout2) clearTimeout(delayedInitTimeout2);
  ...
  // Reacción al trigger del catálogo para añadir capas al mapa
  useEffect(() => {
  ...
  const handleZoomIn = () => {
    if (viewRef.current) {
      viewRef.current.goTo({ zoom: (viewRef.current.zoom || 0) + 1 });
    }
  };
  ...
  const handleMeasure = (type: 'distance' | 'area') => {
    if (widgetsRef.current) {
      widgetsRef.current.measurement.activeTool = type;
    }
  };
    ...
    >
      ...
      <div className={`map-panel floating-panel ${activePanel === 'info' ? 'visible' : 'hidden'}`}>
        ...
        <p className="panel-msg">
          <strong>Cartografía de Cundinamarca</strong>
          <br />
          <br />
